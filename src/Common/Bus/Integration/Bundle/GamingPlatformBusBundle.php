...
final class GamingPlatformBusBundle extends AbstractBundle implements CompilerPassInterface
{
    ...
    public function loadExtension(array $config, ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        foreach ($config['buses'] as $name => $busConfig) {
            $container->services()
                ->set($this->extensionAlias . '.' . $name, RoutingBus::class)
    ...
    public function process(ContainerBuilder $container): void
    {
        foreach ($container->findTaggedServiceIds($this->extensionAlias . '.' . 'handler') as $handlerId => $tags) {
            $this->processHandler($container, $handlerId, $tags);
        }
    }

    /**
     * @param array<int, array{bus?: string, match?: string}> $tags
     */
    private function processHandler(ContainerBuilder $container, string $handlerId, array $tags): void
    {
        foreach ($tags as $tag) {
